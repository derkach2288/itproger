class ProductCollection    # Класс КоллекцияПродуктов, который может считывать все нужные продукты из
                           # подкатегорий films, books и т.д.

    PRODUCT_TYPES = {                         # Создадим константу со всеми возможными типами продуктов. Это ассоциативный
        film: {dir: "films", class: Movie},   # массив, где ключем является символ, а значением — другой ассоцаитивный
        book: {dir: "books", class: Book}     # массив с название папки (где лежат файлы этого типа) и ссылкой на класс.
    }

    def initialize(products = [])   # Конструктор коллекции принимает на вход массив продуктов, но если ничего не
        @products = products     # передали на вход, то он записывает в переменную экземпляра @products пустой массив
    end
    ...
    def self.from_dir(dir_path)
        products = []           # Создаем пустой массив, куда будем складывать все найденные продукты
        ...
        PRODUCT_TYPES.each do |type, hash|
            product_dir = hash[:dir]   # Получим из хэша путь к директории с файлами нужного типа, например, строку 'films'

            product_class = hash[:class]  # Получим из хэша объект нужного класса, например класс Film. Обратите
                                          # внимание, мы оперируем сейчас классом, как объектом. Передаем его по
                                          # ссылки и вызываем у него методы.
  ...
  def to_a   # Простой метод, чтобы достать все продукты коллекции
    ...
    def sort!(params)
        ...
        case params[:by]
        when :title   # Если запросили сортировку по наименованию
        @products.sort_by! { |product| product.to_s }
        when :price   # Если запросили сортировку по цене       
        @products.sort_by! { |product| product.price }
        when :amount   # Если запросили сортировку по количеству       
        @products.sort_by! { |product| product.amount }
        end
        
        @products.reverse! if params[:order] == :asc   # Если запросили сортировку по возрастанию
